	...
	)
{
	...
	if(
		...
		(width > GL_MAX_TEXTURE_SIZE) ||		/*	it's too big, (make sure it's	*/
		(height > GL_MAX_TEXTURE_SIZE) )		/*	2^n for later down-sampling)	*/
	{
		int new_width = 1;
		int new_height = 1;
	...
	if( (width > GL_MAX_TEXTURE_SIZE) || (height > GL_MAX_TEXTURE_SIZE) )
	{
		...
		{
			reduce_block_x = width / GL_MAX_TEXTURE_SIZE;
		}
		if( height > GL_MAX_TEXTURE_SIZE )
		{
			reduce_block_y = height / GL_MAX_TEXTURE_SIZE;
		}
	...
	if( tex_id )
	{
		...
		{
		case 1:
			original_texture_format = channels; //GL_LUMINANCE;
			break;
		case 2:
			original_texture_format = channels; //GL_LUMINANCE_ALPHA;
			break;
		case 3:
...
int query_NPOT_capability( void )
{
	...
	// assuming we're using a GL 2.0+ driver, so npot 2 must be supported
