...
class ProtoGraphics::impl
{
private:
	// No copy
	impl(const impl&); // no implementation 
	impl& operator=(const impl&); // no implementation 

public:
	impl() 
		//: DrawState.blend( blending::SOLID_BLEND )
		//,DrawState.color( glm::vec4(1.f) )
		//,DrawState.move_to( glm::vec2(0.f, 0.f) )
		//,DrawState.xform( identityMatrix )
		//,DrawState.scale( glm::vec3(1.0f) )
	{
		instance = this;
		isRunning = false;
		hasShutdown = false;
	...
	bool init(int xres, int yres, const std::string &resDir ){
		...
		if ( resDir == "" ) {
			setResourceDir( GetExePath() + "../assets/" );
		} else {
		...
		if (glfwOpenWindow(xres,yres,8,8,8,8,24,8,GLFW_WINDOW) == 0 ) return false; // Important to either use 32 bit depth, or 24d8s
		...
		glfwSetWindowSizeCallback( &_sizeCallback );
	...
	void setMouseWheel(int pos) { glfwSetMouseWheel(pos); }
	...
	void moveTo( float x, float y ) { DrawState.move_to = glm::vec2(x,y); }

	void lineTo( float to_x, float to_y )
	{
		LineSegmentState lineSeg = { DrawState.move_to.x, DrawState.move_to.y, DrawState.color, to_x, to_y, DrawState.color };
		buffered_lines.push_back( lineSeg );
		DrawState.move_to = glm::vec2(to_x,to_y); 
	}

	void drawCircle( float x, float y, float radius )
	{
		buffered_circles.push_back( CircleState(DrawState.color,DrawState.blend,x,y,radius) );
	}
	...
	{
		BaseState3D(DrawState.color, DrawState.blend, xform, texture_manager->getActiveTexture(),
	...
	{
		const auto xform = get3DTransform(DrawState.xform, position, glm::vec3(radius));
	...
	void drawMesh( const glm::vec3 &position, float horiz_ang, float verti_ang, const std::string &path )
	{
		...
		glm::mat4 backup = DrawState.xform;
		...
		DrawState.xform = glm::transpose(xrot * yrot);

		// draw, and restore current tform
		drawMesh( position, path );
		DrawState.xform = backup;
	}

	void drawMesh( const glm::vec3 &position, const std::string &path )
	{
		const auto xform = get3DTransform(DrawState.xform, position, DrawState.scale);
		queuePrimitive( xform, MeshManager::getMesh(path) ); // TODO two sided
	}

	// Doesn't use mesh_manager
	void drawMesh( MeshPtr mesh, bool isTwoSided )
	{
		const auto xform = get3DTransform(DrawState.xform, glm::vec3(0.f), DrawState.scale);
		mesh->setIsTwoSided( isTwoSided );
		queuePrimitive( xform, mesh ); // TODO two sided
	}

	void setOrientation( const glm::mat4 &ori ) { DrawState.xform = ori; }

	void setScale( float x, float y, float z ) { DrawState.scale = glm::vec3(x,y,z); }

	void setScale( float uniform_scale ) { DrawState.scale = glm::vec3( uniform_scale ); }

	void setColor( const glm::vec3 &c )
	{
		DrawState.color = glm::vec4(c, DrawState.color.a);
	}

	void setColor( float r, float g, float b )
	{
		DrawState.color.r = r; DrawState.color.b = b; DrawState.color.g = g;
	}
	...
	void setBlend( bool active )
	{
		if ( active )
		{
			DrawState.blend = blending::ALPHA_BLEND;
		}else{
			DrawState.blend = blending::SOLID_BLEND;
		}
	}

	// void disableBlending() { DrawState.blend = blending::SOLID_BLEND; } // SOLIDBLEND (no blend, overwrite)
	// void setAlphaBlend() { DrawState.blend = blending::SOLID_BLEND;		// ALPHABLEND (use alpha channel in image and current color.a (alpha) )
	 void setLightBlend() { DrawState.blend = blending::ADDITIVE_BLEND; }// LIGHTBLEND (additive)
	// void setShadeBlend() { DrawState.blend = blending::SHADE_BLEND; }// SHADEBLEND (multiply with backbuffer
	// void setMaskBlend() { DrawState.blend = blending::MASK_BLEND; }// MASKBLEND (draw if alpha > .5 )
...
private:
	...
	static void _sizeCallback(int w, int h)
	{
		glViewport(0,0,w,h);
		instance->xres = w;
		instance->yres = h;
...
private:
	static impl *instance; // needed for C-style callbacks
	bool isRunning;
	bool hasShutdown;
	...
	int mousx, mousy;
	static const int NUM_KEYS = GLFW_KEY_LAST+1;
...
ProtoGraphics::impl *ProtoGraphics::impl::instance = nullptr;

bool ProtoGraphics::init(int xres, int yres, const std::string &resDir) { return pimpl->init(xres,yres,resDir); }
bool ProtoGraphics::isWindowOpen() { return pimpl->isWindowOpen(); }
glm::ivec2 ProtoGraphics::getWindowDimensions() { return pimpl->getWindowDimensions(); }
double ProtoGraphics::klock() { return pimpl->klock(); }
float ProtoGraphics::getMSPF() { return pimpl->getMSPF(); }
float ProtoGraphics::getAverageMSPF() { return pimpl->getAverageMSPF(); }
void ProtoGraphics::toggleWireframe() { pimpl->toggleWireframe(); }
void ProtoGraphics::debugNormals( bool enable ) { pimpl->debugNormals(enable); }
void ProtoGraphics::reloadShaders() { pimpl->reloadShaders(); }
glm::vec2 ProtoGraphics::getNormalizedMouse() { return pimpl->getNormalizedMouse(); }
int ProtoGraphics::getMouseX() { return pimpl->getMouseX(); }
int ProtoGraphics::getMouseY() { return pimpl->getMouseY(); }
int ProtoGraphics::getMouseWheel() { return pimpl->getMouseWheel(); }
void ProtoGraphics::setMouseWheel(int pos) { pimpl->setMouseWheel(pos); }
bool ProtoGraphics::mouseDownLeft() { return pimpl->mouseDownLeft(); }
bool ProtoGraphics::mouseDownRight() { return pimpl->mouseDownRight(); }
bool ProtoGraphics::keystatus(int key) { return pimpl->keystatus(key); }
bool ProtoGraphics::keyhit(int key) { return pimpl->keyhit(key); }
Ray ProtoGraphics::getMousePickRay() { return pimpl->getMousePickRay(); }
FirstPersonCamera* ProtoGraphics::getCamera() { return pimpl->getCamera(); }
void ProtoGraphics::moveTo( float x, float y ) { pimpl->moveTo(x,y); }
void ProtoGraphics::lineTo( float to_x, float to_y ) { pimpl->lineTo(to_x,to_y); }
...
void ProtoGraphics::setOrientation( const glm::mat4 &ori ) { pimpl->setOrientation(ori); }
void ProtoGraphics::setScale( float x, float y, float z ) { pimpl->setScale(x,y,z); }
void ProtoGraphics::setScale( float uniform_scale ) { pimpl->setScale(uniform_scale); }
void ProtoGraphics::setColor( const glm::vec3 &c ) { pimpl->setColor(c); }
void ProtoGraphics::setColor( float r, float g, float b ) { pimpl->setColor(r,g,b); }
void ProtoGraphics::setAlpha( float a ) { pimpl->setAlpha(a); }
void ProtoGraphics::setBlend( bool active ) { pimpl->setBlend(active); }
void ProtoGraphics::setLightBlend() { pimpl->setLightBlend(); }
void ProtoGraphics::setTexture( const std::string& path ) { pimpl->setTexture(path); }
void ProtoGraphics::disableTexture() { pimpl->disableTexture(); }
void ProtoGraphics::clz() { pimpl->clz(); }
void ProtoGraphics::cls( float r, float g, float b ) { pimpl->cls(r,g,b); }
void ProtoGraphics::frame() { pimpl->frame(); }
void ProtoGraphics::setFrameRate( int frames_per_second ) { pimpl->setFrameRate(frames_per_second); }
void ProtoGraphics::setTitle( const std::string &str) { pimpl->setTitle(str); }
std::string ProtoGraphics::getResourceDir() { return pimpl->getResourceDir(); }
void ProtoGraphics::setResourceDir( const std::string& new_dir ) { pimpl->setResourceDir(new_dir); }
